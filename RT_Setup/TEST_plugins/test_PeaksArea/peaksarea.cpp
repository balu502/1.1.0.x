...
extern "C" PEAKSAREASHARED_EXPORT Test_Interface* __stdcall createTest_plugin()
{
    return(new PeaksArea());
}
...
//---
//-----------------------------------------------------------------------------
void* PeaksArea::Create_Win(void *pobj, void *main)
{
...
//---
//-----------------------------------------------------------------------------
void PeaksArea::Change_TabIndex(int index)
{
...
//--- GetInformation
//-----------------------------------------------------------------------------
void PeaksArea::GetInformation(QVector<QString> *info)
{    
    info->append("0x0030");
...
//--- readCommonSettings()
//-----------------------------------------------------------------------------
void PeaksArea::readCommonSettings()
{
    QString text;
    ...
    CommonSettings->beginGroup("Common");
        ...
        if(translator.load(":/translations/peaksarea_" + text + ".qm"))
        {
...
//--- Set_Test
//-----------------------------------------------------------------------------
void PeaksArea::Set_Test(std::string xml, std::string name)
{
    int i,j,k,m,n;
    int value;
    int id_ch;
    ...
    header_Test->Load_NewTest(QString::fromStdString(name), GetRandomString(), 0x0030);
...
//-----------------------------------------------------------------------------
void PeaksArea::Get_Test(string *xml, string *name)
{
...
//-----------------------------------------------------------------------------
void PeaksArea::fill_Test(rt_Test *ptest)
{
...
//--- create_Common()
//-----------------------------------------------------------------------------
void PeaksArea::create_Common(QWidget *parent)
{
...
//--- create_HeaderTest()
//-----------------------------------------------------------------------------
void PeaksArea::create_HeaderTest(QWidget *parent)
{
    header_Test = new Header_Test(parent);
    active_ch = header_Test->active_Channels;
    header_Test->method_analysis = tr("PeaksArea");
}
//-----------------------------------------------------------------------------
//--- create_Program()
//-----------------------------------------------------------------------------
void PeaksArea::create_Program(QWidget *parent)
{
    Level_Program.clear();
...
//-----------------------------------------------------------------------------
void PeaksArea::Open_Program(QString fn)
{
    int i;
    QList<QTreeWidgetItem*> list_item;
    rt_Protocol *prot_temp;
