...
CyDev::CyDev(HWND winHandle,GUID guid)
{
	...
	//int deviceNumber = 0; // Can be other values if more than 1 device connected to
        ...
        USBMutex=CreateMutex(NULL,FALSE,NULL);
    ...
    //qDebug() << "win_handle" << winHandle;
    //qDebug() << "guid" << guid.Data1 << guid.Data2 << guid.Data3 << guid.Data4;
...
#if 0
void CyDev::CyReset(/*int dir*/)
{
		ULONG nBytes;
		UINT success ;
		///HANDLE fh;

		//if (dir) fh = WriteHandle;
		//else fh = ReadHandle;

	   success = DeviceIoControl(WriteHandle,
				   ...
				   &nBytes,
				   NULL);
		success = DeviceIoControl(ReadHandle,
				   IOCTL_BULKUSB_RESET_PIPE,
				   NULL,
				   0,
				   NULL,
...
void CyDev::Err(char *msg, char *hdr)
{
#ifdef SERVICE_PRG
	LPVOID lpMsgBuf;
	char Msg[200];
	//sprintf(Str,"%s, code: %d",msg,GetLastError());
...
// Free the buffer.
	sprintf(Msg,"%s, code %d, %s",msg,GetLastError(),lpMsgBuf);
	MessageBox(0,Msg,hdr,MB_OK|MB_ICONINFORMATION);
	LocalFree( lpMsgBuf );
#endif
	return;
}
...
bool CyDev::BulkClear(void )
{
	...
	saveEvent = OvStr.hEvent;
	OvStr.hEvent = NULL;

	bRes=ReadFile(ReadHandle, buf, 64, &rlen, &OvStr);
	bRes=CancelIo(ReadHandle);

	OvStr.hEvent=saveEvent;

	/*GetOverlappedResult(
        ReadHandle,	// handle of file, pipe, or communications device
        &OvStr,	// address of overlapped structure
        &BytesRead,	// address of actual bytes count
        FALSE 	// wait flag
        );
